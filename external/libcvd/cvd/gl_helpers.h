...
namespace CVD
{
	...
	///@ingroup gGL
	template <class C>
	inline void glDrawPixels(const SubImage<C> &i)
	{
	...
	/// @ingroup gGL
	template <class C>
	inline void glTexSubImage2D(const SubImage<C> &i, GLint xoffset = 0, GLint yoffset = 0, GLenum target = GL_TEXTURE_2D, GLint level = 0)
	{
	...
	/// @ingroup gGL
	template <class C>
	inline void glTexImage2D(const SubImage<C> &i, GLint border = 0, GLenum target = GL_TEXTURE_2D, GLint level = 0)
	{
		::glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		::glPixelStorei(GL_UNPACK_ROW_LENGTH, i.row_stride());
