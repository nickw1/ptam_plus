...
/* Computing MAX */
		...
		for (j = max(i__4,1); j <= i__3; ++j) {
		    tmp += (r__1 = ab[i__ + 1 - j + j * ab_dim1], dabs(r__1)) 
			    * (r__2 = x[j + k * x_dim1], dabs(r__2)); /* L60: */ 		} /* Computing MIN */ 		i__4 = i__ + *kd; 		i__3 = min(i__4,*n); 		for (j = i__; j <= i__3; ++j) { 		    tmp += (r__1 = ab[j + 1 - i__ + i__ * ab_dim1], dabs(r__1) 			    ) * (r__2 = x[j + k * x_dim1], dabs(r__2));
/* L70: */
		}
	    }
	    if (i__ == 1) {
