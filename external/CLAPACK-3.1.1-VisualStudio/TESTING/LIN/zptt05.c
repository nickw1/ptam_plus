	...
	doublereal *berr, doublereal *reslts)
{
    ...
    static integer imax;
    static doublereal unfl, ovfl, xnorm;
...
/*     Test 2:  Compute the maximum of BERR / ( NZ*EPS + (*) ), where   
	...
	if (*n == 1) {
		...
		tmp = (d__1 = b[i__5].r, abs(d__1)) + (d__2 = d_imag(&b[i__ + 
			...
			d__5 = x[i__7].r, abs(d__5)) + (d__6 = d_imag(&x[i__  			- 1 + k * x_dim1]), abs(d__6))) + ((d__7 = z__1.r,  			abs(d__7)) + (d__8 = d_imag(&z__1), abs(d__8))) + (( 			d__9 = e[i__8].r, abs(d__9)) + (d__10 = d_imag(&e[i__] 			), abs(d__10))) * ((d__11 = x[i__9].r, abs(d__11)) + (
