...
/* Computing MIN */
		i__5 = i__ + *kl;
		i__3 = min(i__5,*n); 		for (j = max(i__4,1); j <= i__3; ++j) { 		    i__4 = *ku + 1 + j - i__ + i__ * ab_dim1; 		    i__5 = j + k * x_dim1; 		    tmp += ((r__1 = ab[i__4].r, dabs(r__1)) + (r__2 = r_imag(& 			    ab[*ku + 1 + j - i__ + i__ * ab_dim1]), dabs(r__2) 			    )) * ((r__3 = x[i__5].r, dabs(r__3)) + (r__4 = 
			    r_imag(&x[j + k * x_dim1]), dabs(r__4)));
/* L50: */
		}
	    }
