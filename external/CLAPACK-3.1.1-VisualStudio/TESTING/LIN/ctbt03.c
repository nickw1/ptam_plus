...
/* Subroutine */ int ctbt03_(char *uplo, char *trans, char *diag, integer *n,  	integer *kd, integer *nrhs, complex *ab, integer *ldab, real *scale,  	real *cnorm, real *tscal, complex *x, integer *ldx, complex *b,  	integer *ldb, complex *work, real *resid  	)
{
...
/*     Compute the norm of the triangular matrix A using the column   
    ...
    if (lsame_(diag, "N")) {
	if (lsame_(uplo, "U")) {
	    i__1 = *n; 	    for (j = 1; j <= i__1; ++j) { /* Computing MAX */ 		r__1 = tnorm, r__2 = *tscal * c_abs(&ab[*kd + 1 + j * ab_dim1] 			) + cnorm[j];
