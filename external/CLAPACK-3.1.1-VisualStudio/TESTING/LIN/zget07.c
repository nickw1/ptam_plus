...
/* Subroutine */ int zget07_(char *trans, integer *n, integer *nrhs,  	doublecomplex *a, integer *lda, doublecomplex *b, integer *ldb,  	doublecomplex *x, integer *ldx, doublecomplex *xact, integer *ldxact,  	doublereal *ferr, doublereal *berr, doublereal *reslts  	)
{
    ...
    static doublereal eps, tmp, diff, axbi;
    static integer imax;
...
/*     Test 2:  Compute the maximum of BERR / ( (n+1)*EPS + (*) ), where   
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
		...
		for (j = 1; j <= i__3; ++j) {
		    i__4 = i__ + j * a_dim1;
		    i__5 = j + k * x_dim1; 		    tmp += ((d__1 = a[i__4].r, abs(d__1)) + (d__2 = d_imag(&a[ 			    i__ + j * a_dim1]), abs(d__2))) * ((d__3 = x[i__5] 			    .r, abs(d__3)) + (d__4 = d_imag(&x[j + k * x_dim1] 			    ), abs(d__4)));
/* L40: */
		}
	    } else {
		i__3 = *n;
		for (j = 1; j <= i__3; ++j) {
		    i__4 = j + i__ * a_dim1;
		    i__5 = j + k * x_dim1; 		    tmp += ((d__1 = a[i__4].r, abs(d__1)) + (d__2 = d_imag(&a[ 			    j + i__ * a_dim1]), abs(d__2))) * ((d__3 = x[i__5] 			    .r, abs(d__3)) + (d__4 = d_imag(&x[j + k * x_dim1] 			    ), abs(d__4)));
/* L50: */
		}
	    }
	    if (i__ == 1) {
