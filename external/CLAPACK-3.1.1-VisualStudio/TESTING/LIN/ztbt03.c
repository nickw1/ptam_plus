...
/* Subroutine */ int ztbt03_(char *uplo, char *trans, char *diag, integer *n, 
	integer *kd, integer *nrhs, doublecomplex *ab, integer *ldab,  	doublereal *scale, doublereal *cnorm, doublereal *tscal,  	doublecomplex *x, integer *ldx, doublecomplex *b, integer *ldb,  	doublecomplex *work, doublereal *resid  	)
{
...
/*     Compute the norm of the triangular matrix A using the column   
    ...
    if (lsame_(diag, "N")) {
	if (lsame_(uplo, "U")) {
	    i__1 = *n; 	    for (j = 1; j <= i__1; ++j) { /* Computing MAX */ 		d__1 = tnorm, d__2 = *tscal * z_abs(&ab[*kd + 1 + j * ab_dim1] 			) + cnorm[j];
		tnorm = max(d__1,d__2);
/* L10: */
	    }
	} else {
