	...
	iwork, logical *bwork, integer *info)
{
    ...
    static doublereal v, eps, tol, tmp, ulp;
    static integer sdim, kmin, itmp, ipnt[20], rsub;
    static char sort[1];
    static integer sdim1;
...
/*        Do Test (13) */
	...
	if (isort == 1) {
		...
		if (i__ < *n) {
		    d__1 = -wi[i__ + 1];
		    d__2 = -wi[i__];
		    if ((dslect_(&wr[i__ + 1], &wi[i__ + 1]) || dslect_(&wr[ 			    i__ + 1], &d__1)) && ! (dslect_(&wr[i__], &wi[i__] 			    ) || dslect_(&wr[i__], &d__2)) && iinfo != *n + 2)
			     {
			result[13] = ulpinv;
		    }
		}
