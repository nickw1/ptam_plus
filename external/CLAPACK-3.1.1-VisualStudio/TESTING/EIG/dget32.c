	...
	integer *knt)
{
    ...
    /* Local variables */
    static doublereal b[4]	/* was [2][2] */, x[4]	/* was [2][2] */;
    static integer n1, n2, ib;
    static doublereal tl[4]	/* was [2][2] */, tr[4]	/* was [2][2] */;
    static integer ib1, ib2, ib3;
    static doublereal den, val[3], eps;
    static integer itl;
    static doublereal res, sgn;
    static integer itr;
    static doublereal tmp;
    static integer info, isgn;
    static doublereal tnrm, xnrm, scale, xnorm;
			...
			for (itr = 1; itr <= 3; ++itr) {
				...
				for (ib2 = 1; ib2 <= 3; ++ib2) {
				    ...
				    res += (d__1 = (tl[3] + sgn * tr[0]) * x[
					    ...
					    abs(d__1)); 				    tnrm = abs(tr[0]) + abs(tl[0]) + abs(tl[2] 					    ) + abs(tl[1]) + abs(tl[3]);
/* Computing MAX */
		...
		for (itr = 1; itr <= 8; ++itr) {
			...
			for (itl = 1; itl <= 3; ++itl) {
				...
				for (ib2 = 1; ib2 <= 3; ++ib2) {
					...
					tr[2] = tr[1];
					tr[1] = tmp;
				    } 				    tnrm = abs(tl[0]) + abs(tr[0]) + abs(tr[2] 					    ) + abs(tr[3]) + abs(tr[1]);
...
/* Computing MAX */
					    d__1 = abs(x[0]) + abs(x[2]), 
						    d__2 = abs(x[1]) + abs(x[
						    3]);
					    xnrm = max(d__1,d__2); 					    res = (d__1 = (tl[0] + sgn * tr[0] 						    ) * x[0] + sgn * tr[1] * 
						    x[2] + tl[2] * x[1] - 
						    scale * b[0], abs(d__1));
