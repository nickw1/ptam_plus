...
/* Computing MIN */
			  ...
			  tl[1] = tmp;
					    ...
					    tnrm = dabs(tr[0]) + dabs(tr[1])  						    + dabs(tr[2]) + dabs(tr[3] 						    ) + dabs(tl[0]) + dabs(tl[
						    1]) + dabs(tl[2]) + dabs(
						    tl[3]);
/* Computing MAX */
					    r__1 = dabs(x[0]) + dabs(x[2]), 
						    r__2 = dabs(x[1]) + dabs(
						    x[3]);
					    xnrm = dmax(r__1,r__2); 					    res = (r__1 = (tl[0] + sgn * tr[0] 						    ) * x[0] + sgn * tr[1] * 
						    x[2] + tl[2] * x[1] - 
						    scale * b[0], dabs(r__1));
