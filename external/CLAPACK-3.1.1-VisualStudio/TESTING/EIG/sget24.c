	...
	work, integer *lwork, integer *iwork, logical *bwork, integer *info)
{
    ...
    static real v, eps, tol, tmp, ulp;
    static integer sdim, kmin, itmp, ipnt[20], rsub;
    static char sort[1];
    static integer sdim1, iinfo;
...
/*        Do Test (13) */
	...
	if (isort == 1) {
		...
		if (i__ < *n) {
		    r__1 = -wi[i__ + 1];
		    r__2 = -wi[i__];
		    if ((sslect_(&wr[i__ + 1], &wi[i__ + 1]) || sslect_(&wr[ 			    i__ + 1], &r__1)) && ! (sslect_(&wr[i__], &wi[i__] 			    ) || sslect_(&wr[i__], &r__2)) && iinfo != *n + 2)
			     {
			result[13] = ulpinv;
		    }
		}
